...
iv = os.urandom(16)
cA = AES.new(k, AES.MODE_CBC, iv)
cB = AES.new(k, AES.MODE_CBC, iv)

m0 = b"Hi Bob"
m1 = b"Hi Alice"

c0 = cA.encrypt(pad(m0, AES.block_size))
c1 = cB.encrypt(pad(m1, AES.block_size))

dA = AES.new(k, AES.MODE_CBC, iv)
dB = AES.new(k, AES.MODE_CBC, iv)

m0_decrypted = unpad(dA.decrypt(c0), AES.block_size)
m1_decrypted = unpad(dB.decrypt(c1), AES.block_size)
